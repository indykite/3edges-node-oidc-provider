...
module.exports = function getClient(provider)
{
  ...
  class Client
  {
    ...
    encryptText(value, secret = process.env.INTERNAL_SECRET)
    {
      return CryptoJS.AES.encrypt(value, secret);
    }

    decryptText(value, secret = process.env.INTERNAL_SECRET)
    {
    ...
    {
      let a = this.clientSecret;
      const b = actual;

      if (a.startsWith('U2F'))
      {
        a = this.decryptText(this.clientSecret, process.env.INTERNAL_SECRET);
      }
